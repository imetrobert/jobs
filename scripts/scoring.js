...
A posting whose own "Location" field already names Montreal or anywhere in that gazetteer is "onsite_close" or "onsite_far" by that fact alone — no quote needed, the location field IS the evidence, and it is NEVER "not_montreal" (which by definition requires the office to be OUTSIDE the greater Montreal area — a Montreal address cannot satisfy that). The quote requirement below belongs to "remote_montreal" only; do not apply it here.
